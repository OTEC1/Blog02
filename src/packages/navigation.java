...
public void setGistq(String gistq) {
	...
	public  String onload(HttpServletRequest reqHttpSession) {
		
		HttpSession  session=reqHttpSession.getSession();
     if( session.isNew()){
		 String address=read_ip(1);
		 new ServerRequest().push_in_visitor_ip(address,1);
		  reqHttpSession.setAttribute("counts", "fill");
		 }
return null;
}
